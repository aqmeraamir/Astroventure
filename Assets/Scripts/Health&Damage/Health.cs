...
public class Health : MonoBehaviour
{
    ...
    /// void (no return)
    /// </summary>
    private GravityPickup gravityPickup;
    void Start()
    {
        gravityPickup = GameObject.FindObjectOfType<GravityPickup>();
...
{   
    ...
    bool IsGravityInverted = GravityPickup.isPickedUp;

    Debug.Log(IsGravityInverted);
    if (playerController != null && IsGravityInverted) {
        playerController = GetComponent<PlayerController>();
        // Reset gravity if the player has a GravityPickup component
